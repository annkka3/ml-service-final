...
async def init(drop_all: Optional[bool] = None) -> None:
    ...
    Инициализация БД.
    drop_all:
      - True  -> перед созданием таблиц удаляет все
      - False -> только создаёт недостающие таблицы
      - None  -> берёт поведение из settings.INIT_DB_DROP_ALL
    """
    if drop_all is None:
        drop_all = settings.INIT_DB_DROP_ALL

    # --- Создание/очистка схемы ---
